...
- 尚无窗口边缘吸附和角色模式切换。
- 尚无前台窗口感知、系统音频检测、LLM 工具调用、TTS、音乐或羁绊系统。
- M11 对话只保存在当前窗口内存中，不形成长期记忆；模型不能控制角色、窗口或原生命令。
...
- [x] 可替换 Provider 的流式 LLM 对话壳与离线 Mock 降级；尚未接入 Agent 工具。
- [ ] 可选 TTS/语音能力。
...
- 模型通信通过 Provider 接口隔离；M11 只处理文本对话，M12 才能把经过白名单和调度器校验的语义工具暴露给模型。
