...
> 文档状态：P0-P4 已完成；P5 仅剩内置备份恢复人工复核
>
> 决策日期：2026-07-16
>
> 上位计划：[`计划.md`](../计划.md) M8
...
P5 当前状态（2026-07-21）：兼容声明、保存前备份与恢复、脱敏诊断、供应链记录、第三方 notices、性能基线、bundle 边界、Windows 单实例、便携程序和 NSIS 安装包均已实现并通过自动化或真实 release 运行验证。开发态界面、严格 CSP 正式构建及除内置备份恢复按钮外的 Windows 人工发布矩阵均由用户确认通过；备份入口说明已补入发布文档。完成一次“诊断 → 项目备份 → 恢复”的人工复核后即可封板，实际结果见 [`M8-P5验收报告.md`](./M8-P5验收报告.md)。
